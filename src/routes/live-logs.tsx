        ...
        <div className="grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
          <Card><CardHeader><CardTitle>Managed streams</CardTitle></CardHeader><CardContent className="p-0">
            <div className="space-y-2 p-4">
              {targets.map((runner) => (
                <button className={`w-full rounded-md border p-3 text-left transition-colors ${runner.id === selectedId ? "border-primary/40 bg-primary/5" : "border-border hover:bg-muted/40"}`} key={runner.id} onClick={() => selectTarget(runner.id)} type="button">
          ...
          <Card className="min-w-0 overflow-hidden">
            ...
            <CardContent className="relative p-0">
              ...
              <pre aria-live="polite" className="min-h-[540px] max-h-[68vh] overflow-auto border-t border-border bg-[#070b0b] p-4 font-mono text-[11px] leading-5 text-emerald-100/80" onScroll={handleLogScroll} ref={logViewport}><code>{logs || (loading ? "Loading runner output…" : "No output yet.")}</code></pre>
