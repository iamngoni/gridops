...
import { Activity, ArrowLeft, CircleAlert, CircleCheck, CircleX, LoaderCircle, Save, Server, Settings2, TriangleAlert } from "lucide-react";
        ...
        {pool.canManage ? <form className="mt-6 space-y-4" onSubmit={submit}>
          ...
          <Card>
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              <Field label="Target runners" hint="Runners GridOps should keep active now. Autoscaling may change this between the minimum and maximum."><Input max="100" min="0" name="desiredCount" onChange={(event) => setDesiredCount(Number(event.target.value))} required type="number" value={desiredCount} /></Field>
...
function PoolActivityRow({ event }: { event: RunnerPoolEvent }) {
  ...
  return <div className="grid gap-2 px-6 py-4 sm:grid-cols-[auto_1fr_auto] sm:items-start">
    ...
    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-medium">{event.event}</span><Badge variant={event.level === "error" ? "destructive" : event.level === "warning" ? "outline" : "secondary"}>{event.level}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{event.message}</p>{capacity ? <div className="mt-3 rounded-md border border-amber-500/25 bg-amber-500/5 px-3 py-2 text-xs text-foreground"><div className="font-medium text-amber-800 dark:text-amber-100">Current manager capacity snapshot</div><div className="mt-1 grid gap-x-4 gap-y-1 text-muted-foreground sm:grid-cols-3"><span>{capacity.active.cpu.toLocaleString()} / {capacity.cpuBudget.toLocaleString()} CPU cores used</span><span>{capacity.active.memoryMb.toLocaleString()} / {capacity.memoryBudgetMb.toLocaleString()} MB used</span><span>{capacity.active.activeRunners} active / {capacity.maxRunners} runner limit</span></div><p className="mt-2 text-[11px] leading-4 text-muted-foreground">New provisioning is blocked until this active capacity falls below the host guardrails.</p></div> : null}{event.runnerId ? <p className="mt-1 font-mono text-[11px] text-muted-foreground">Runner {event.runnerId}</p> : null}{event.metadata && event.metadata !== "{}" ? <details className="group mt-2"><summary className="cursor-pointer text-[11px] text-primary/80 hover:text-primary">View event details</summary><pre className="mt-2 max-h-48 overflow-auto rounded-md bg-muted/40 p-3 text-[11px] leading-5 text-foreground/80">{metadata}</pre></details> : null}</div>
