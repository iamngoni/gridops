              ...
              return (
                <TableRow key={run.id}>
                  ...
                  <TableCell><div className="flex justify-end gap-1">
                    {run.canManage && active ? <AsyncActionButton action={() => control({ data: { runId: run.id, action: "cancel" } })} confirm={`Cancel ${run.workflowName} #${run.runNumber}?`} icon={<Square />} size="icon" success="Cancellation requested." title={`Cancel ${run.workflowName}`}><span className="sr-only">Cancel run</span></AsyncActionButton> : null}
                    {run.canManage && active ? <AsyncActionButton action={() => control({ data: { runId: run.id, action: "force-cancel" } })} confirm={`Force-cancel ${run.workflowName} #${run.runNumber}? Use this when normal cancellation is blocked by a running condition.`} icon={<OctagonX />} size="icon" success="Force cancellation requested." title={`Force-cancel ${run.workflowName}`} variant="ghost"><span className="sr-only">Force-cancel run</span></AsyncActionButton> : null}
                    {run.canManage && !active ? <AsyncActionButton action={() => control({ data: { runId: run.id, action: "rerun" } })} icon={<RotateCcw />} size="icon" success="Workflow rerun requested." title={`Rerun ${run.workflowName}`}><span className="sr-only">Rerun workflow</span></AsyncActionButton> : null}
                    {run.canManage && run.conclusion === "failure" ? <AsyncActionButton action={() => control({ data: { runId: run.id, action: "rerun-failed" } })} icon={<RefreshCw />} size="icon" success="Failed jobs rerun requested." title={`Rerun failed jobs in ${run.workflowName}`}><span className="sr-only">Rerun failed jobs</span></AsyncActionButton> : null}
                    <a aria-label="Open run on GitHub" className="inline-flex size-8 items-center justify-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground" href={String(run.htmlUrl)} rel="noreferrer" target="_blank" title="Open run on GitHub"><ExternalLink className="size-4" /></a>
                  </div></TableCell>
                </TableRow>
              );
            })}</TableBody>
